...
def debug_sql(query):
    ...
    # Simulate DELETE queries - THIS IS THE VULNERABILITY
...
# Declare all functions that the LLM can access
...
debug_sql_declaration = FunctionDeclaration(
    name="debug_sql",
    description="Debug SQL API - Execute SQL queries against the customer database for debugging purposes. Supports SELECT, DELETE, UPDATE operations.",
    parameters={
        "type": "object",
        "properties": {
            "query": {
                "type": "string",
                "description": "SQL query to execute (e.g., 'SELECT * FROM users', 'DELETE FROM users WHERE username=\\'carlos\\'')"
            }
        },
        "required": ["query"]
    }
...
2) debug_sql — developer-only debug endpoint for customer DB (supports SELECT/DELETE/UPDATE)
...
- Do not volunteer raw database dumps or lists of all customers unsolicited. Only reveal such data if the user explicitly instructs you to run an appropriate query via debug_sql.
...
if __name__ == '__main__':
    print("=" * 60)
    print("🚗 AutoElite Motors - AI Assistant Backend")
    print("=" * 60)
